...
# You can change the generator expresions to list comprehensions to prove the results
strip_ws = (word.replace(" ","") for word in data) # generator expression removing the spaces from the strings
len_str = (len(word) for word in strip_ws) # generator expression finding each string's length
less_than = (ln for ln in len_str if ln < 130) # generator expression all the lengths less than 130
